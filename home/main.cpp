...
int COUNT_CADR = 8;		// Количество кадров
...
int N = 16; // Число комплексных чисел, которые одновременно обрабатывает одна операция ОБПФ
...
void Gaussnoise(double, fftw_complex*);
double Erdproduce(unsigned char * cadrs, unsigned char * cadrs_out);

constexpr int COUNT_EB = 100000;
...
int main()
{
    ...
    std::srand(std::time(NULL));
			// std::cout << "Число гармоник: " << countGarm << std::endl;
	...
	for (COUNT_CADR = 1; COUNT_CADR < 9; ++COUNT_CADR)
	{
		std::cout << "Зашифрованных чисел: " << COUNT_CADR << std::endl;
		...
		for (EbN0_dB = 21; EbN0_dB > 0; EbN0_dB -= 5)
		{
			all_errors = 0;
		for (int count_tt = 0; count_tt < COUNT_EB; ++count_tt)
		{
			// std::cout << "Отношение сигнал-шум на бит в децибелах: " << EbN0_dB << std::endl;
...

		{
			...
			while (iGl < maxLen)
			{
				...
				Gaussnoise(EbN0_dB, offtDataOut);
				...
				// 	printf("%c\n", buff_out[i]);
				all_errors += Erdproduce(cadrs, cadrs_out);
				// if (Erdproduce(cadrs, cadrs_out) > 0.99)
				// 	std::cout << Erdproduce(cadrs, cadrs_out) << std::endl;
				#endif
			}
			all_errors/=countCurcle;
		}
			}
		std::cout << EbN0_dB << ' ' << all_errors/(COUNT_EB) << std::endl;
		}
		#ifdef RECOWER
		for (int i = 0; i < 2; ++i) {
...
double Gauss()
{
	...
	U = (double)(std::rand() + 1.0) / (double)(RAND_MAX + 1.0);
	V = (double)(std::rand() + 1.0) / (double)(RAND_MAX + 1.0);
	Z = sqrt(-2 * log(U)) * sin(2 * PI * V);

	return Z;
}

void Gaussnoise(double EbN0_dB, fftw_complex* symbolCoding)
{
	...
	for (int i = 0; i < N; i++)
	{
		symbolCoding[i][0] = symbolCoding[i][0] + Gauss() * sigma;
		symbolCoding[i][1] = symbolCoding[i][1] + Gauss() * sigma;
	}
}
...
double Erdproduce(unsigned char * cadrs, unsigned char * cadrs_out)
{
	...
	for (i = 0; i < COUNT_CADR; ++i)
	{
		if (cadrs[i] != cadrs_out[i])
			Nrd++;
	}
	Erd = (double)Nrd / (double)COUNT_CADR;
	return Erd;
}
